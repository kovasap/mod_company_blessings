...
WorldTownScreenTempleDialogModule.prototype.isConnected =
    function() { return this.mSQHandle !== null; };
...
WorldTownScreenTempleDialogModule.prototype.onConnection = function(_handle) {
  ...
  if (this.mEventListener !== null &&
      ('onModuleOnConnectionCalled' in this.mEventListener)) {
...
WorldTownScreenTempleDialogModule.prototype.onDisconnection = function() {
  ...
  // notify listener
  if (this.mEventListener !== null &&
      ('onModuleOnDisconnectionCalled' in this.mEventListener)) {
...
WorldTownScreenTempleDialogModule.prototype.createDIV = function(_parentDiv) {
  ...
  this.mContainer =
      $('<div class="l-temple-dialog-container display-none opacity-none"/>');
  _parentDiv.append(this.mContainer);
  this.mDialogContainer =
      this.mContainer.createDialog('Temple', '', '', true, 'dialog-1024-768');
  ...
  this.mNoInjuredLabel = $(
      '<div class="is-no-wounded-hint text-font-medium font-bottom-shadow font-color-description display-none">You have no wounded in your ranks that can be treated here.</div>');
  ...
  this.mDetailsPanel.Container =
      $('<div class="details-container display-none"/>');
  ...
  var detailsColumn =
      $('<div class="column is-character-portrait-container"/>');
  detailsRow.append(detailsColumn);
  this.mDetailsPanel.CharacterImage =
      detailsColumn.createImage(null, function(_image) {
        var offsetX = 0;
        var offsetY = 0;
  ...
  var explanation = $(
      '<div class="text-font-medium font-bottom-shadow font-color-description is-explanation">Have wounds treated so that they heal faster and don\'t worsen while on the road.</div>');
  ...
  this.mDetailsPanel.CharacterName = $(
      '<div class="name title-font-normal font-bold font-color-brother-name"/>');
  backgroundRow.append(this.mDetailsPanel.CharacterName);
  backgroundRow = $('<div class="row is-bottom"/>');
  detailsColumn.append(backgroundRow);

  this.mDetailsPanel.ScrollContainer =
      backgroundRow.createList(10, 'is-injury-list', true);
  this.mDetailsPanel.ScrollContainerList =
      this.mDetailsPanel.ScrollContainer.findListScrollContainer();
  ...
  this.mLeaveButton = layout.createTextButton(
      "Leave", function() { self.notifyBackendLeaveButtonPressed(); }, '', 1);
...
WorldTownScreenTempleDialogModule.prototype.addListEntry = function(_data) {
  ...
  image.bindTooltip({
    ...
    elementId : TooltipIdentifier.CharacterBackgrounds.Generic,
    elementOwner : TooltipIdentifier.ElementOwner.HireScreen,
    entityId : _data.ID
  });

  var name = $(
      '<div class="name title-font-normal font-bold font-color-brother-name">' +
      _data[WorldTownScreenIdentifier.HireRosterEntry.Name] + '</div>');
  row.append(name);
  ...
  for (var i = 0; i < _data.Injuries.length; ++i) {
    ...
    icon.bindTooltip({
      ...
      entityId : _data.ID,
      statusEffectId : _data.Injuries[i].id
    });
    row.append(icon);
  }
};

WorldTownScreenTempleDialogModule.prototype.addBlessingListEntry = function(_data) {
  var result = $('<div class="l-row"/>');
  this.mListScrollContainer.append(result);
...
WorldTownScreenTempleDialogModule.prototype.updateSelectedListEntry = function(
    _data) {
...
WorldTownScreenTempleDialogModule.prototype.selectListEntry = function(
    _element, _scrollToEntry) {
...
WorldTownScreenTempleDialogModule.prototype.updateDetailsPanel = function(
    _element) {
  if (_element !== null && _element.length > 0) {
    var currentMoney = this.mAssets.getValues().Money;
    var data = _element.data('entry');

    this.mDetailsPanel.CharacterImage.attr('src',
                                           Path.PROCEDURAL + data.ImagePath);
    ...
    for (var i = 0; i < data.Injuries.length; ++i) {
      this.createInjuryControlDIV(i, this.mDetailsPanel.ScrollContainerList,
                                  data.ID, data.Injuries[i], currentMoney);
    }

    var idx = 0;
    // TODO use js for this loop, find a way to propagate the constant
    foreach(blessingID, blessing in ::COMPANY_BLESSINGS.Blessings) {
      this.createBlessingControlDIV(idx, this.mDetailsPanel.ScrollContainerList,
                                    blessingID, currentMoney);
      idx++;
    }

    this.mDetailsPanel.Container.removeClass('display-none')
        .addClass('display-block');
  } else {
    this.mDetailsPanel.Container.removeClass('display-block')
        .addClass('display-none');
  }
};

WorldTownScreenTempleDialogModule.prototype.createInjuryControlDIV = function(
    _i, _parentDiv, _entityID, _data, _money) {
  var self = this;
  ...
  icon.bindTooltip({
    contentType : 'status-effect',
    entityId : _entityID,
    statusEffectId : _data.id
  });
  row.append(icon);

  var name = $(
      '<div class="is-name text-font-normal font-bottom-shadow font-color-description">' +
      _data.name + '</div>');
  row.append(name);
  ...
  if (_data.price > _money)
    price = $(
        '<div class="is-price"><div class="is-price-ffs font-color-assets-negative-value"><img src="' +
        Path.GFX + Asset.ICON_MONEY_SMALL + '"/> ' +
        Helper.numberWithCommas(_data.price) + '</div></div>');
  else
    price = $('<div class="is-price"><div class="is-price-ffs"><img src="' +
              Path.GFX + Asset.ICON_MONEY_SMALL + '"/> ' +
              Helper.numberWithCommas(_data.price) + '</div></div>');
...
WorldTownScreenTempleDialogModule.prototype.createBlessingControlDIV = function(
    _i, _parentDiv, _blessingID, _blessing, _money) {
  var self = this;
  ...
  var icon = $('<img class="is-icon"/>');
  // TODO get perk icon here
  icon.attr('src', Path.GFX + _blessing.);
  icon.bindTooltip({
    contentType : 'status-effect',
    entityId : _entityID,
    statusEffectId : _data.id
  ...
  var button = layout.createCustomButton(price, function() {
    self.notifyBackendRecieveBlessing(_blessingID, function(_result) {
      self.mAssets.loadFromData(_result.Assets);
      self.updateSelectedListEntry(_result.Entity);
      self.updateDetailsPanel(self.mSelectedEntry);
    });
...
WorldTownScreenTempleDialogModule.prototype.bindTooltips = function() {
  ...
  this.mLeaveButton.bindTooltip({
    contentType : 'ui-element',
    elementId : TooltipIdentifier.WorldTownScreen.HireDialogModule.LeaveButton
  });
};
...
WorldTownScreenTempleDialogModule.prototype.register = function(_parentDiv) {
  ...
  if (this.mContainer !== null) {
    console.error(
        'ERROR: Failed to register World Town Screen Temple Dialog Module. Reason: Module is already initialized.');
    return;
  }
...
WorldTownScreenTempleDialogModule.prototype.unregister = function() {
  ...
  if (this.mContainer === null) {
    console.error(
        'ERROR: Failed to unregister World Town Screen Temple Dialog Module. Reason: Module is not initialized.');
    return;
  }
...
WorldTownScreenTempleDialogModule.prototype.registerEventListener = function(
    _listener) { this.mEventListener = _listener; };

WorldTownScreenTempleDialogModule.prototype.show = function(
    _withSlideAnimation) {
  var self = this;

  var withAnimation =
      (_withSlideAnimation !== undefined && _withSlideAnimation !== null)
          ? _withSlideAnimation
          : true;
  if (withAnimation === true) {
    var offset = -(this.mContainer.parent().width() + this.mContainer.width());
    this.mContainer.css({'left' : offset});
    this.mContainer.velocity("finish", true)
        .velocity({opacity : 1, left : '0', right : '0'}, {
...
WorldTownScreenTempleDialogModule.prototype.hide = function() {
  ...
  var offset = -(this.mContainer.parent().width() + this.mContainer.width());
  this.mContainer.velocity("finish", true)
      .velocity({opacity : 0, left : offset}, {
        duration : Constants.SCREEN_SLIDE_IN_OUT_DELAY,
        easing : 'swing',
        begin : function() {
...
WorldTownScreenTempleDialogModule.prototype.isVisible =
    function() { return this.mIsVisible; };

WorldTownScreenTempleDialogModule.prototype.updateAssets =
    function(_data) {
...
WorldTownScreenTempleDialogModule.prototype.changeRosterEntry = function(
    _entryID) {
  var self = this;
...
WorldTownScreenTempleDialogModule.prototype.notifyBackendModuleShown =
    function() { SQ.call(this.mSQHandle, 'onModuleShown'); };

WorldTownScreenTempleDialogModule.prototype.notifyBackendModuleHidden =
    function() { SQ.call(this.mSQHandle, 'onModuleHidden'); };

WorldTownScreenTempleDialogModule.prototype.notifyBackendModuleAnimating =
    function() { SQ.call(this.mSQHandle, 'onModuleAnimating'); };

WorldTownScreenTempleDialogModule.prototype.notifyBackendLeaveButtonPressed =
    function() { SQ.call(this.mSQHandle, 'onLeaveButtonPressed'); };

WorldTownScreenTempleDialogModule.prototype.notifyBackendBrothersButtonPressed =
    function() { SQ.call(this.mSQHandle, 'onBrothersButtonPressed'); };

WorldTownScreenTempleDialogModule.prototype.notifyBackendTreatInjury = function(
    _entityID, _injuryID, _callback) {
  SQ.call(this.mSQHandle, 'onTreatInjury', [ _entityID, _injuryID ], _callback);
};

WorldTownScreenTempleDialogModule.prototype.notifyBackendRecieveBlessing = function(
    _blessingID, _callback) {
  SQ.call(this.mSQHandle, 'onRecieveBlessing', [ _blessingID ], _callback);
};
